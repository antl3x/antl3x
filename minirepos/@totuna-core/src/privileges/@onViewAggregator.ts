...
satisfies<module, typeof import("./@onViewAggregator.js")>;

export interface module extends atAggregator<_StateSchema, Record<string, AggregateObject>, "onView"> {}
...
export const genAggregatesFiles: module["genAggregatesFiles"] = (aggregates) => {
  ...
  for (const key in aggregates) {
    ...
    const fileName = `${_meta_.schema}.${_meta_.view}.ts`;
    ...
    const content = `import { AggregateObject } from "@totuna/core/privileges/@onViewAggregator";\n
...
export const filesToAggregates: module["filesToAggregates"] = async (files) => {
  ...
  for (const [path] of files) {
    ...
    const key = `${_meta_.schema}.${_meta_.view}`;
